...
struct PubKeyData {
    ...
    init(secItems: SecItemsProvider, secKeys: SecKeysProvider) {
        self.secItems = secItems
        self.secKeys = secKeys
    }
    
    func queryKeychain(alias: String) throws -> (rawData: Data, dict: [String: Any]) {
        ...
        if keyClass == (kSecAttrKeyClassPublic as String) {
            ...
            guard let extractedPub = secKeys.copyPublicKey(keyRef) else {
        ...
        guard let rawPubKeyData = secKeys.copyExternalRepresentation(publicKey, &exportError) as Data? else {
